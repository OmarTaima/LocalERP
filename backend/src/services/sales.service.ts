...
export async function enrichLines(tenantId: string, lines: OrderLineInput[]): Promise<OrderLine[]> {
  return Promise.all(
    lines.map(async (line) => {
      const product = await ProductModel.findOne({ _id: line.productId, tenantId });
...
async function requireCustomer(tenantId: string, customerId: string): Promise<CustomerDoc> {
...
export async function listCustomers(tenantId: string, query: { search?: string; status?: string; page?: number; pageSize?: number }) {
...
export async function createCustomer(tenantId: string, userId: string, input: Record<string, unknown>) {
  const doc = await CustomerModel.create({ tenantId, ...input, totalSpent: 0, status: "active" });
  const after = serializeCustomer(doc);
  await writeAudit({ tenantId, userId, action: "create", entity: "Customer", entityId: doc._id.toString(), after: { name: doc.name, email: doc.email } });
  publish({ type: "sales.customer.created", payload: { tenantId, customerId: doc._id.toString() } });
  return after;
}
...
export async function updateCustomer(tenantId: string, userId: string, customerId: string, input: Record<string, unknown>) {
  const doc = await CustomerModel.findOne({ _id: customerId, tenantId });
  if (!doc) throw new AppError(404, "customer not found");
  ...
  await doc.save();
  const after = serializeCustomer(doc);
  await writeAudit({ tenantId, userId, action: "update", entity: "Customer", entityId: customerId, before, after: { name: doc.name, email: doc.email, status: doc.status } });
  return after;
}

export async function listQuotes(tenantId: string, query: { status?: string; customerId?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, query.page ?? 1);
  const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 20));
  const filter: Record<string, unknown> = { tenantId };
  if (query.status) filter.status = query.status;
...
export async function createQuote(tenantId: string, userId: string, input: { customerId: string; items: OrderLineInput[]; validUntil: string }) {
  await requireCustomer(tenantId, input.customerId);
  const items = await enrichLines(tenantId, input.items);
  const doc = await QuoteModel.create({
    tenantId,
    quoteNumber: await nextNumber(tenantId, "quote", "QT"),
  ...
  await writeAudit({ tenantId, userId, action: "create", entity: "Quote", entityId: doc._id.toString(), after: { quoteNumber: doc.quoteNumber, total: doc.totals.total } });
  publish({ type: "sales.quote.created", payload: { tenantId, quoteId: doc._id.toString(), quoteNumber: doc.quoteNumber } });
  return serializeQuote(doc);
}

export async function getQuote(tenantId: string, quoteId: string) {
  const doc = await QuoteModel.findOne({ _id: quoteId, tenantId });
  if (!doc) throw new AppError(404, "quote not found");
  return serializeQuote(doc);
}

export async function updateQuote(tenantId: string, userId: string, quoteId: string, input: { items?: OrderLineInput[]; validUntil?: string; status?: "sent" | "declined" }) {
  const doc = await QuoteModel.findOne({ _id: quoteId, tenantId });
  if (!doc) throw new AppError(404, "quote not found");
  ...
  if (input.items) {
    ...
    doc.items = await enrichLines(tenantId, input.items);
  ...
  await doc.save();
  await writeAudit({ tenantId, userId, action: "update", entity: "Quote", entityId: quoteId, before, after: { status: doc.status, total: doc.totals.total } });
  return serializeQuote(doc);
}

export async function convertQuote(tenantId: string, userId: string, quoteId: string) {
  const doc = await QuoteModel.findOne({ _id: quoteId, tenantId });
  if (!doc) throw new AppError(404, "quote not found");
  ...
  const customer = await CustomerModel.findOne({ _id: doc.customerId, tenantId });
  ...
  const order = await SalesOrderModel.create({
    tenantId,
    orderNumber: await nextNumber(tenantId, "order", "ORD"),
  ...
  doc.version += 1;
  await doc.save();
  await writeAudit({ tenantId, userId, action: "update", entity: "Quote", entityId: quoteId, after: { status: "converted" } });
  publish({ type: "sales.quote.converted", payload: { tenantId, quoteId, orderId: order._id.toString() } });
  return serializeOrder(order);
}

export async function listOrders(tenantId: string, query: { status?: string; customerId?: string; from?: string; to?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, query.page ?? 1);
  const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 20));
  const filter: Record<string, unknown> = { tenantId };
  if (query.status) filter.status = query.status;
...
export async function createOrder(tenantId: string, userId: string, input: {
  customerId: string;
  quoteId?: string;
  ...
  const existing = await SalesOrderModel.findOne({ tenantId, idempotencyKey: input.idempotencyKey });
  if (existing) return { ...serializeOrder(existing), duplicate: true };
  await requireCustomer(tenantId, input.customerId);
  if (input.quoteId) {
    const quote = await QuoteModel.findOne({ _id: input.quoteId, tenantId });
  ...
  await writeAudit({ tenantId, userId, action: "create", entity: "SalesOrder", entityId: doc._id.toString(), after: { orderNumber: doc.orderNumber, total: doc.totals.total } });
  publish({ type: "sales.order.created", payload: { tenantId, orderId: doc._id.toString(), orderNumber: doc.orderNumber } });
  return serializeOrder(doc);
}

export async function getOrder(tenantId: string, orderId: string) {
  const doc = await SalesOrderModel.findOne({ _id: orderId, tenantId });
  if (!doc) throw new AppError(404, "order not found");
  const [payments, shipments, rmas] = await Promise.all([
    PaymentModel.find({ tenantId, orderId: doc._id }).sort({ createdAt: 1 }).lean(),
    ShipmentModel.find({ tenantId, orderId: doc._id }).sort({ createdAt: 1 }).lean(),
    RmaModel.find({ tenantId, orderId: doc._id }).sort({ createdAt: 1 }).lean(),
  ]);
  return {
    ...serializeOrder(doc),
    timeline: {
...
export async function updateOrderStatus(tenantId: string, userId: string, orderId: string, status: OrderStatus) {
  const doc = await SalesOrderModel.findOne({ _id: orderId, tenantId });
  if (!doc) throw new AppError(404, "order not found");
  ...
  doc.version += 1;
  await doc.save();
  await writeAudit({ tenantId, userId, action: "update", entity: "SalesOrder", entityId: orderId, before, after: { status: doc.status } });
  publish({ type: "sales.order.status_changed", payload: { tenantId, orderId, status: doc.status } });
  return serializeOrder(doc);
}

export async function createPayment(tenantId: string, userId: string, orderId: string, input: { amount: number; method: "card" | "transfer" | "cash" | "refund"; reference?: string; idempotencyKey: string }) {
  const existing = await PaymentModel.findOne({ tenantId, idempotencyKey: input.idempotencyKey });
  if (existing) return { ...serializePayment(existing), duplicate: true };
  const order = await SalesOrderModel.findOne({ _id: orderId, tenantId });
  ...
  const captured = await PaymentModel.aggregate([
    { $match: { tenantId: new mongoose.Types.ObjectId(tenantId), orderId: order._id, status: "captured" } },
  ...
  const customer = await CustomerModel.findOne({ _id: order.customerId, tenantId });
  if (customer) {
  ...
  await writeAudit({ tenantId, userId, action: "create", entity: "Payment", entityId: doc._id.toString(), after: { amount: doc.amount, method: doc.method } });
  publish({ type: "sales.payment.captured", payload: { tenantId, orderId, paymentId: doc._id.toString(), amount: doc.amount } });
...
export async function listShipments(tenantId: string, query: { status?: string; orderId?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, query.page ?? 1);
  const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 20));
  const filter: Record<string, unknown> = { tenantId };
  if (query.status) filter.status = query.status;
...
export async function createShipment(tenantId: string, userId: string, input: { orderId: string; carrier: string; trackingNumber: string; pickList: Array<{ productId: string; quantity: number; fromWarehouseId: string }> }) {
  const order = await SalesOrderModel.findOne({ _id: input.orderId, tenantId });
  if (!order) throw new AppError(404, "order not found");
  ...
  for (const line of input.pickList) {
    const product = await ProductModel.findOne({ _id: line.productId, tenantId });
    if (!product) throw new AppError(400, `product ${line.productId} does not exist`);
    const warehouse = await WarehouseModel.findOne({ _id: line.fromWarehouseId, tenantId, isActive: true });
  ...
  await writeAudit({ tenantId, userId, action: "create", entity: "Shipment", entityId: doc._id.toString(), after: { carrier: doc.carrier, status: "draft" } });
  publish({ type: "sales.shipment.created", payload: { tenantId, shipmentId: doc._id.toString(), orderId: input.orderId } });
  return serializeShipment(doc);
}

export async function updateShipmentStatus(tenantId: string, userId: string, shipmentId: string, input: { status: "packed" | "shipped" | "delivered"; trackingNumber?: string }) {
  const doc = await ShipmentModel.findOne({ _id: shipmentId, tenantId });
  ...
  await doc.save();
  const order = await SalesOrderModel.findOne({ _id: doc.orderId, tenantId });
  if (order) {
  ...
  await writeAudit({ tenantId, userId, action: "update", entity: "Shipment", entityId: shipmentId, after: { status: doc.status, trackingNumber: doc.trackingNumber } });
  if (doc.status === "shipped") publish({ type: "sales.shipment.shipped", payload: { tenantId, shipmentId, orderId: doc.orderId.toString() } });
  return serializeShipment(doc);
}

export async function listRmas(tenantId: string, query: { status?: string; orderId?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, query.page ?? 1);
  const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 20));
  const filter: Record<string, unknown> = { tenantId };
  if (query.status) filter.status = query.status;
...
export async function createRma(tenantId: string, userId: string, input: { orderId: string; items: Array<{ productId: string; quantity: number; batchId?: string; condition: string }>; reason: string }) {
  const order = await SalesOrderModel.findOne({ _id: input.orderId, tenantId });
  if (!order) throw new AppError(404, "order not found");
  ...
  const doc = await RmaModel.create({
    tenantId,
    rmaNumber: await nextNumber(tenantId, "rma", "RMA"),
    orderId: order._id,
    items: input.items,
    reason: input.reason,
    status: "requested",
  });
  await writeAudit({ tenantId, userId, action: "create", entity: "Rma", entityId: doc._id.toString(), after: { rmaNumber: doc.rmaNumber, reason: doc.reason } });
  publish({ type: "sales.rma.requested", payload: { tenantId, rmaId: doc._id.toString(), orderId: input.orderId } });
  return serializeRma(doc);
}

export async function updateRmaStatus(tenantId: string, userId: string, rmaId: string, status: "approved" | "rejected" | "received" | "refunded") {
  const doc = await RmaModel.findOne({ _id: rmaId, tenantId });
  ...
  doc.status = status;
  await doc.save();
  await writeAudit({ tenantId, userId, action: "update", entity: "Rma", entityId: rmaId, after: { status: doc.status } });
  if (status === "refunded") publish({ type: "sales.rma.refunded", payload: { tenantId, rmaId, orderId: doc.orderId.toString() } });
  return serializeRma(doc);
}

export async function listRecurringInvoices(tenantId: string, query: { status?: string; customerId?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, query.page ?? 1);
  const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 20));
  const filter: Record<string, unknown> = { tenantId };
  if (query.status) filter.status = query.status;
...
export async function createRecurringInvoice(tenantId: string, userId: string, input: { customerId: string; items: Array<{ productId?: string; name: string; quantity: number; unitPrice: number }>; interval: "weekly" | "monthly" | "quarterly" | "yearly"; dayOfPeriod: number }) {
  await requireCustomer(tenantId, input.customerId);
  ...
  await writeAudit({ tenantId, userId, action: "create", entity: "RecurringInvoice", entityId: doc._id.toString(), after: { interval: doc.interval, nextRunAt: doc.nextRunAt.toISOString() } });
...
export async function updateRecurringStatus(tenantId: string, userId: string, recurringId: string, status: "active" | "paused" | "cancelled") {
  const doc = await RecurringInvoiceModel.findOne({ _id: recurringId, tenantId });
  if (!doc) throw new AppError(404, "recurring invoice not found");
  doc.status = status;
  await doc.save();
  await writeAudit({ tenantId, userId, action: "update", entity: "RecurringInvoice", entityId: recurringId, after: { status: doc.status } });
  return serializeRecurring(doc);
}

export async function runDueRecurringInvoices(tenantId: string): Promise<number> {
  const due = await RecurringInvoiceModel.find({ tenantId, status: "active", nextRunAt: { $lte: new Date() } });
  let generated = 0;
  for (const recurring of due) {
    ...
    if (lines.length > 0) {
      const items = await enrichLines(tenantId, lines);
  ...
  if (generated > 0) publish({ type: "sales.recurring.invoices_generated", payload: { tenantId, count: generated } });
  return generated;
}