...
| logoUrl | string | nullable; Cloudinary secure delivery URL (`res.cloudinary.com`) — uploaded by the browser directly via unsigned preset `ml_default`; set via `POST /company/logo` or `POST /admin/companies/:id/logo` |
...
| avatarUrl | string | nullable; Cloudinary secure delivery URL — uploaded by the browser directly via unsigned preset `ml_default`; set via `POST /auth/avatar`, or `avatarUrl` on user create/update |
| isActive | boolean | default true |
...
| images | string[] | Cloudinary secure delivery URLs (browser-uploaded via unsigned preset `ml_default`); an `image` URL on create/update replaces the array (replace semantics) |
